...
def CH_Geo(Profile:str, Nozzle_Type:str, Ac_At:float=10, f_pct:float=1,Phi_i:float=45, Phi_n:float=45, Phi_e:float=None, Phi_ex:float=5,Twall:float=5, export:str = "CSV", plotEnable:bool = False, Grid = "Coarse",t_fin: float = 0.0):
    ...
    L_post = 0.1*Dc
    #%% Pre-and Post-CC
    if export == "OF" or export == "OF_axs":
        L_post = 0.5*Dc  #For now just a simple rule of thumb 0.5 (0.1)
        
    elif export =="CSV":
        L_post = 0.1*Dc  #For now just a simple rule of thumb 0.5 (0.1)
    ...
    if Nozzle_Type == 'Conical' : 
        ...
        y_entr = 1.5*Dt/2*np.sin(Phirange1 * np.pi/180)+ 1.5*Dt/2 + Dt/2
        #exit section
        Phirange2 = np.arange(0 , Phi_e , 1)  #(-90 , Phi_e-90 , 1)
        x_exit = 0.382*Dt/2 * np.sin(Phirange2* np.pi/180) #0.382*Dt/2*np.cos(Phirange2 * np.pi/180)       
        y_exit = Dt/2 + 0.382*Dt/2 *(1-np.cos(Phirange2* np.pi/180)) #0.382*Dt/2*np.sin(Phirange2 * np.pi/180)+15*Dt/2 + Dt/2
        Nx = 0.382*Dt/2 * np.sin(Phi_e* np.pi/180) #1.5*Dt/2*np.cos((Phi_e-90) * np.pi/180)
        Ny = Dt/2 + 0.382*Dt/2 *(1-np.cos(Phi_e* np.pi/180))#1.5*Dt/2*np.sin((Phi_e-90)* np.pi/180)+1.5*Dt/2 + Dt/2
        #Quadratic Bézier parabola curve
        m1 = np.tan(Phi_e*np.pi/180)
        m2 = np.tan(Phi_ex*np.pi/180)
        C1 = Ny - m1 * Nx
        C2 = De/2 - m2 * Ln
        Qx = (C2-C1)/(m1-m2)
        Qy = m1*Qx+C1
        ...
        y_combinedsup = [x + Twall*1e-3 for x in y_combined]
        y_combinedsup_fin = [x + t_fin*1e-3 for x in y_combinedsup]

            #,np.append(y_coord,y_inlet)

        ContourCoordInt = pd.DataFrame({"x": x_combined, "y": y_combined, "z": [0]*len(x_combined)})
        ContourCoordIntsup = pd.DataFrame({"x": x_combined, "y": y_combinedsup, "z": [0]*len(x_combined)})
        ContourCoordIntsupFin = pd.DataFrame({"x": x_combined, "y": y_combinedsup_fin, "z": [0]*len(x_combined)})
        ContourCoordInt = ContourCoordInt.sort_values("x", ignore_index=True)
        ContourCoordIntsup = ContourCoordIntsup.sort_values("x", ignore_index=True)
        ContourCoordIntsupFin = ContourCoordIntsupFin.sort_values("x", ignore_index=True)
        ContourCoordInt = ContourCoordInt.drop_duplicates("x", ignore_index=True)
        ContourCoordIntsup = ContourCoordIntsup.drop_duplicates()
        ContourCoordIntsupFin = ContourCoordIntsupFin.drop_duplicates()
    ...
    if export == "CSV":
       ...
       # Create a new DXF document
        ...
        fin_points = [(row["x"], row["y"]) for _, row in ContourCoordIntsupFin.iterrows()]
        ...
        # Add spline for the outer fin contour
        if len(fin_points) > 1:
            msp.add_spline(fin_points, degree=3)
        ...
        x_start_fin, y_start_fin = fin_points[0]
        x_end_fin, y_end_fin = fin_points[-1]
        ...
        msp.add_line((x_start_outer, y_start_outer), (x_start_fin, y_start_fin))  # Start connection
        msp.add_line((x_end_outer, y_end_outer), (x_end_fin, y_end_fin))          # End connection
        ...
        split_indices = [100, 170]

        # Function to split points list based on indices and include end points as start of next segment
        def split_points_by_indices(points, indices):
            segments = []
            start = 0
            for index in indices:
                segments.append(points[start:index+1])
                start = index  # Include the end point as the start of the next segment
            segments.append(points[start:])  # Add the remaining points
            return segments

        # Split inner_points into segments
        inner_segments = split_points_by_indices(inner_points, split_indices)

        # Add splines for each segment of the inner contour
        for segment in inner_segments:
            if len(segment) > 1:  # Ensure there are enough points for a spline
                msp.add_spline(segment, degree=3)  # Cubic spline (degree=3) for smoothness
...
'''for i in [2.5,5.0,7.5,10.0]:
    HRE_1_Geo = CH_Geo(HRE_1, 'Bell', 10, 1 ,Phi_i=45, Phi_n=35,Phi_e = 30, Phi_ex=1,Twall=i, export ="DXF_sim")'''
HRE_1_Geo = CH_Geo(HRE_1, 'Bell', 10, 1 ,Phi_i=45, Phi_n=35,Phi_e = 30, Phi_ex=5,Twall=10, export ="DXF_sim",plotEnable=True, t_fin=15)
